...
def draw_graph_from_matrix(matrix_str, edges=None, path_color='red', colors=None):
    ...
    if edges:
        ...
        for i in range(num_nodes):
            for j in range(i, num_nodes):  # Use (i, j) with j >= i for undirected graph
                weight = matrix[i][j]
                if weight != 0:
    ...
    nx.draw(graph, pos, ax=ax, with_labels=True, node_color=node_colors,
            ...
            font_weight="bold", edge_color=path_color, width=2)
...
if __name__ == '__main__':
    ...
    draw_graph_from_matrix(adjacency_matrix_str, path_color='gray')
